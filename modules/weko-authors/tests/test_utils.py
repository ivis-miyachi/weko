...
def test_check_email_existed(esindex,mocker):
    body1 = {"emailInfo":[{"email":"test@test.org"}],"gather_flg":0,"pk_id":"1"}
    body2 = {"emailInfo":[{"email":"not_test@test.org"}],"gather_flg":0,"pk_id":"2"}
    esindex.index(index=current_app.config["WEKO_AUTHORS_ES_INDEX_NAME"],id=1,body=body1)
    esindex.index(index=current_app.config["WEKO_AUTHORS_ES_INDEX_NAME"],id=2,body=body2)
    esindex.indices.refresh()
    #data={"hits":{"total":{"value":1},"hits":[{"_source":{"pk_id":1}}]}}
    ...
    #mock_indexer.client.return_value=data
    result = check_email_existed("testx@test.org")
    assert result == {"email":"testx@test.org","author_id":""}
...
def test_get_count_item_link(app,esindex):
    body1={"author_link":["1","2"],"control_number":"1"}
    body2={"author_link":["2","3"],"control_number":"2"}
    esindex.index(index=current_app.config["SEARCH_UI_SEARCH_INDEX"],id=1,body=body1)
    esindex.index(index=current_app.config["SEARCH_UI_SEARCH_INDEX"],id=2,body=body2)
    esindex.indices.refresh()

    result = get_count_item_link("1")
    assert result == 1
    result = get_count_item_link("4")
    assert result == 0
