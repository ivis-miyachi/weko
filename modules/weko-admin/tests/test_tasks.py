...
def test_clean_temp_info(instance_path, mocker):
    ...
    dir_export_file = os.path.join(instance_path,"export_file")
    ...
    os.makedirs(dir_export_file, exist_ok=True)
    
    export_info = {
            "expire":(datetime.now()+timedelta(days=-10)).strftime("%Y-%m-%d %H:%M:%S"),
            "is_export":True,
            "uri": "test_uri",
    ...
    with patch("weko_search_ui.utils.delete_exported") as mock_delete_exporeted:
        clean_temp_info()
        ...
        mock_delete_exporeted.assert_called_with(dir_export_file,export_info)
