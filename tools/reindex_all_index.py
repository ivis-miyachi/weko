...
template_files = {
    "events-stats-index": ("invenio_stats", f"contrib/events/{version}/events-v1.json"),
    "stats-index": ("invenio_stats", f"contrib/aggregations/{version}/aggregations-v1.json"),
}
...
for index in indexes:
    ...
    with open(file_path, "r") as json_file:
        ...
        if mapping_type == "templates":
            if index_tmp not in templates:
                templates[index_tmp] = json.loads(json_file.read())
...
try:
    new_event_stats_index_name = os.environ.get('SEARCH_INDEX_PREFIX')+"-events-stats-index-000001"
    template_url_event_stats = template_url.format(new_event_stats_index_name+"/"+version)
    res = requests.put(template_url,json=templates[new_event_stats_index_name.replace("-000001","")],**req_args)
...
# statsインデックスの集約
new_stats_index_name = os.environ.get('SEARCH_INDEX_PREFIX')+"-stats-index-000001"
template_url_stats = template_url.format(new_stats_index_name+"/"+version)
res = requests.put(template_url,json=templates[new_stats_index_name.replace("-000001","")],**req_args)
